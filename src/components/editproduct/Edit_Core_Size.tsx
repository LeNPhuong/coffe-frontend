...
import Edit_Core_Size_Items from "./Edit_Core_Size_Items";

const Edit_Core_Size: React.FC<{
        ...
        <div className="my-[10px] flex flex-col gap-[10px]">
            ...
            <div className="flex flex-row px-[16px]">
                <div className="flex flex-row justify-between w-full">
                    {data.map((e) => (
                        <Edit_Core_Size_Items key={e._id} data={e} size={size} setSize={setSize} />
                    ))}
                </div>
            </div>
        </div>
